...
void CalculateConstraintBounds(const MatrixXd& contact_state, double fz_max,
                               ...
                               int horizon, std::vector<double> constraint_tightening_flattened,
                               VectorXd* constraint_lb_ptr,VectorXd* constraint_ub_ptr);
...
class ConvexMpc {
 public:
  ...
  ConvexMpc(double mass, const std::vector<double>& inertia, int num_legs,
            ...
            const std::vector<double>& qp_weights, const std::vector<double>& control_weight_vec,
            QPSolverName qp_solver_name = QPOASES,
            double fz_min_scale=0.0, double fz_max_scale=10.0);
  ...
  // Previous dynamics A matrix
  const MatrixXd& GetPreviousAMat() const {return a_exp_;}

  // Previous dynamics B matrix
  const MatrixXd& GetPreviousBMat() const {return b_exp_;}

  // Mean of previous foot positions in world frame
  const MatrixXd& GetPreviousFootPositionsWorld() const {return foot_positions_world_;}
  
  // Get qp unsolved flag
  const bool GetQPUnsolved() const {return qp_unsolved;}
 ...
 private:
  ...
  // Minimum scaling factor for force in Z direction
  double fz_min_scale_;

  // Maximum scaling factor for force in Z direction
  double fz_max_scale_;

  // Flag if failed to solve QP
  bool qp_unsolved;
};
...
void CalculateConstraintBounds(const MatrixXd& contact_state, double fz_max,
  ...
  for (int i = 0; i < horizon; ++i) {
    for (int j = 0; j < num_legs; ++j) {
      ...
      constraint_lb(row) = 0 + constraint_tightening_flattened[row];
      constraint_lb(row + 1) = 0 + constraint_tightening_flattened[row+1];
      constraint_lb(row + 2) = 0 + constraint_tightening_flattened[row+2];
      constraint_lb(row + 3) = 0 + constraint_tightening_flattened[row+3];
      constraint_lb(row + 4) = fz_min * contact_state(i, j) + constraint_tightening_flattened[row+4];
...
ConvexMpc::ConvexMpc(double mass, const std::vector<double>& inertia,
                     ...
                     const std::vector<double>& qp_weights, const std::vector<double>& control_weight_vec,
                     QPSolverName qp_solver_name, double fz_min_scale, double fz_max_scale)
    : mass_(mass),
      inv_mass_(1 / mass),
      ...
      alpha_(AsBlockDiagonalMat(control_weight_vec,planning_horizon)),
      alpha_single_(AsBlockDiagonalMat(control_weight_vec,1)),
      ...
      workspace_(0),
      initial_run_(true),
      fz_min_scale_(fz_min_scale),
      fz_max_scale_(fz_max_scale),
      qp_unsolved(false)
{
...
std::vector<double> ConvexMpc::ComputeContactForces(
  ...
  // Reset unsolved qp status
  qp_unsolved = false;

  std::vector<double> error_result;

  // Each row represents the foot contact states for
  // that time instant within the planning horizon
  ...
  DCHECK_EQ(foot_contact_states.cols(), num_legs_);
  ...
  for (int i = 0; i < planning_horizon_; ++i) {
    ...
    // Desired yaw - weight for this is set to zero because config desired is 0
    ...
    // Prefer to stablize roll and pitch. (desired yaw is 0)
    ...
    // Track X,Y velocity but maintain the desired Z height
  ...
  CalculateConstraintBounds(foot_contact_states, mass_ * kGravity * fz_max_scale_,
                            mass_ * kGravity * fz_min_scale_,
                            foot_friction_coeffs[0], planning_horizon_,
                            constraint_tightening_flattened,
  ...
  if (qp_solver_name_ == OSQP) {
    ...
    return qp_solution_; //save this result for computing constraint tightening
  } else {
    ...
    qpOASES::returnValue status  =  qp_problem.init(red_hessian.data(), red_g_vec.data(), red_a_mat.data(),
                    nullptr, nullptr, red_a_lb.data(), red_a_ub.data(),
                    max_solver_iter);
    
    // If QP failed to solve, determine why
    if (status != qpOASES::SUCCESSFUL_RETURN)
    {
      std::cout<<"Failed to solve QP due to reason: "<<status<<"\n";
      qp_unsolved = true;
    }
...
PYBIND11_MODULE(mpc_osqp, m) {
  ...
  py::class_<ConvexMpc>(m, "ConvexMpc")
      .def(py::init<double, const std::vector<double>&, int, int, double,
                    const std::vector<double>&, const std::vector<double>&, QPSolverName, double, double>())
      .def("compute_contact_forces", &ConvexMpc::ComputeContactForces) //also previous mpc solution
      .def("reset_solver", &ConvexMpc::ResetSolver)

      .def("get_previous_amat",&ConvexMpc::GetPreviousAMat)

      // Data needed for chance-constraint tightening computation

      //previous dynamics B matrix
      .def("get_previous_bmat", &ConvexMpc::GetPreviousBMat)

      //previous foot positions mean in world frame
      .def("get_previous_foot_positions_world", &ConvexMpc::GetPreviousFootPositionsWorld)

      //get success failure criteria based on if qp unsolved
      .def("get_qp_unsolved",&ConvexMpc::GetQPUnsolved);
