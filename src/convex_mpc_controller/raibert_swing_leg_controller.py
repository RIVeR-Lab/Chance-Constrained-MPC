...
cfg = controller_configs.get_raibert_swing_leg_controller_conf()
...
class RaibertSwingLegController:
    ...
    def __init__(self,
        ...
        Args:
            ...
            foot_landing_clearance:The foot clearance on the ground at the end of the swing cycle.
            foot_height: Apex of the swing leg
        """
        ...
        self.use_raibert_heuristic = cfg.use_raibert_heuristic
    ...
    def reset(self, current_time: float) -> None:
        ...
        self._phase_switch_foot_local_position = self._robot.foot_positions_in_base_frame.copy()
    ...
    def update(self, current_time: float) -> None:
        ...
        for leg_id, state in enumerate(new_leg_state):
            if (state == gait_generator_lib.LegState.SWING and state != self._last_leg_state[leg_id]):
                self._phase_switch_foot_local_position[leg_id] = (self._robot.foot_positions_in_base_frame[leg_id])
    ...
    def get_action(self, desired_speed, desired_twisting_speed) -> Mapping[Any, Any]:
        ...
        for leg_id, leg_state in enumerate(self._gait_generator.leg_state):
            ...
            if self.use_raibert_heuristic or (not self.foot_placement_position.any()):
                ...
                foot_target_position = (hip_horizontal_velocity * self._gait_generator.stance_duration[leg_id] / 2 
                                        - cfg.k_raibert *(target_hip_horizontal_velocity - hip_horizontal_velocity))
                
                foot_target_position = np.clip(foot_target_position,[-0.15, -0.1, -0.05], [0.15, 0.1, 0.05])
                
                foot_target_position = foot_target_position - self._desired_landing_height + np.array((hip_offset[0], hip_offset[1], 0))
                
            else:
                foot_target_position = self.foot_placement_position[leg_id] - self._desired_landing_height + np.array((hip_offset[0], hip_offset[1], 0))
            ...
            joint_ids, joint_angles = (self._robot.get_motor_angles_from_foot_position(leg_id, foot_position))
