...
def joint_angles_from_link_position(
    ...
    base_position, base_orientation = robot.pybullet_client.multiplyTransforms(
        robot.base_position, robot.base_orientation_quat, #B->W
        base_translation,base_rotation) #B'->B, identity transformation
    
    
    # Projects link position to world space.
    world_link_pos, _ = robot.pybullet_client.multiplyTransforms(
        base_position, base_orientation, #B->W
        link_position, _IDENTITY_ORIENTATION) #L->B
    
    ik_solver = 0 #IK solver- Damped Least Squares
    
    # All the joint angles due to the position of the given leg in world space
...
def link_position_in_base_frame(robot: typing.Any,link_id: int,):
    ...
    """
    # W->B
    inverse_translation, inverse_rotation = robot.pybullet_client.invertTransform(robot.base_position, robot.base_orientation_quat)
    ...
    # L->B
    link_local_position, _ = robot.pybullet_client.multiplyTransforms(inverse_translation, inverse_rotation, link_position, (0, 0, 0, 1))
...
def compute_jacobian(robot: typing.Any,link_id: int,):
    ...
    # Setting up zero vector for joint velocities and desired joint accelerations
    zero_vec = list(np.zeros_like(all_joint_angles).astype(np.float32))
    
    # Compute the 3 X N jacobian matrix
    jv, _ = robot.pybullet_client.calculateJacobian(robot.quadruped, 
                                                    link_id,
                                                    (0., 0., 0.),
                                                    all_joint_angles,
                                                    zero_vec,
    ...
    # Extract the position jacobian(3 X 18)
