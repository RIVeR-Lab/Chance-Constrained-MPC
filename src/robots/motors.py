...
class MotorModel:
    ...
    def __init__(
        ...
        init_position: float = 0.0, #rad
        min_position: float = 0.0, #rad
        max_position: float = 0.0, #rad
        min_velocity: float = 0.0, #rad/s
        max_velocity: float = 0.0, #rad/s
        min_torque: float = 0.0, #N-m
    ...
    def __init__(self, motors: Tuple[MotorModel, ...] = ()) -> None:
        ...
        for motor in motors:
            if motor0_control_mode != motor._motor_control_mode:
                raise ValueError("Using different control mode for different motors is not currently supported.")
